...
- 证照：供应商、经营主体或业务流程中需要审核的资质类材料，例如食品经营许可证、食品生产许可证、营业执照和烟草专卖零售许可证。
- 批次报告：围绕商品批次、生产日期、检测结论等信息形成的质量或合规报告。
- 第三方检验报告：由外部检验机构出具、用于证明商品或批次满足检测要求的报告。
- 核验项：一条可执行、可解释、可测试的确定性审核条件，例如主体名称一致、统一社会信用代码一致或有效期覆盖审核日期。
- 审核结论：平台基于字段抽取、规则校验和人工复核状态输出的结构化结果，包括风险等级、规则结果、摘要和是否需要人工复核。
- 人工复核：当材料无法识别、关键字段缺失、规则结果异常或存在中高风险时，由审核人员进行的人工判断。
- 规则执行器：执行一组确定性规则、收集规则状态、聚合风险等级并映射为平台 `RuleResult` 的通用基础设施。
- workflow runtime：位于 `app/workflows/` 的流程编排层，负责组织文档加载、字段抽取、规则执行、风险汇总和人工复核路由。
...
| Python 规则引擎 | 执行确定性规则校验，输出规则状态、规则结果和风险等级；通用规则协议、上下文、执行器和聚合逻辑放在 `app/rules/`，具体规则放在各 Skill 的 `rules/` |

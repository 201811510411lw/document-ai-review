...
- 自动一致性审核通常按字段差异数量决定 OA 流转：`0..2` 项普通字段不匹配时当前机器人节点返回 `pass` 并进入下一节点复核，`>=3` 项时返回 `reject`。字段差异只统计证照类型、许可证号、主体名称、经营地址、负责人和有效期等业务字段；子审核状态、证据完整性等过程规则不计入字段差异数量。烟草证明确已过期属于硬性拒绝条件，即使总差异少于 3 项也返回 `reject`。
- `pass`、`reject` 回调均必须包含 `mismatch_count`、阈值和结构化 `field_differences`，逐项给出字段名、字段中文名、营业执照侧/期望值、烟草证侧/实际值、差异类型和规则信息，供 OA 下一节点展示和复核。`reject` 回调还必须包含完整 `reject_reasons`，每项保留规则编码、规则名称、明确原因和 `details`；自动拒绝摘要应说明问题总数。
- 证据缺失、候选冲突、临近到期等非技术问题随规则明细提交下一节点；数据库、NAS、OCR、LLM、持久化或 RPA 技术故障返回 `exception`。官网真伪核验明确失败仍可直接返回 `reject`，不参与字段差异阈值计算。
...
- 一致性校验完成后，由上层一致性审核流程使用烟草证许可证号统一发起一次影刀 RPA 官网验真。

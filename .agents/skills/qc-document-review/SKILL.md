...
- `declared_document_type=batch_report` 时进入商品批次报告链路。
...
- 报告中的产品名称需与来源系统商品名一致。
- 比对前忽略空白和常见标点差异。
- 缺失或不一致时进入人工复核，风险等级 `MEDIUM`。

### 生产者名称

- 报告中的厂名、公司名、生产商或生产单位需与来源系统供应商名称一致。
- 比对前忽略空白和常见标点差异。
- 缺失或不一致时进入人工复核，风险等级 `MEDIUM`。

### 生产日期或生产批号

- 报告中的生产日期需与来源系统批次明细 `production_date` 一致。
- 若报告只识别到生产批号，且批号中包含来源生产日期的 `YYYYMMDD` 数字串，也可视为匹配。
- 缺失或不一致时进入人工复核，风险等级 `MEDIUM`。

...
- `subject_name` 有识别值时，必须具备 `subject_name_evidence`，且 evidence 应为证照图片/PDF 页面中的可见原文片段。
- `credit_code` 有识别值时，必须具备 `credit_code_evidence`，且 evidence 应为证照图片/PDF 页面中的可见原文片段。
- 若关键字段识别值存在但对应 evidence 缺失，不允许输出“无风险”自动通过，必须进入人工复核，原因输出“关键字段缺少 OCR 原文证据”。
- 该规则失败时风险等级为 `MEDIUM`。
...
- 主体名称缺失或明显不一致。
- 统一社会信用代码缺失、格式异常或不一致。
- 主体名称或统一社会信用代码有识别值但缺少对应 OCR 原文证据。
- 有效期有值但无法解析。
- OCR/LLM 证据不足以支持自动通过。

## 烟草证一致性审核中的材料角色

- 单店只使用一张营业执照，该执照主体名称、负责人、经营地址需与烟草证一致，主体名称还需与 OA 提供的加盟商名称一致。
- 店中店使用两张营业执照：烟草持证主体营业执照与烟草证核对主体名称、负责人和地址；加盟店营业执照不与烟草证核对主体名称和负责人，只参与证照类型、字段证据和同址校验。
- 不得按上传顺序猜测两张营业执照角色。无法通过主体名称和负责人唯一识别烟草持证主体时进入人工复核。

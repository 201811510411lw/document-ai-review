...
- `document_type_raw`：从证照图片上能直接看到的证照大标题文字，例如"食品经营许可证""小餐饮经营许可证""仅销售预包装食品备案凭证""仅销售预包装食品备案""网络食品交易第三方平台备案""食品小经营店登记证""小食杂店登记证""食品摊贩备案卡"。优先取页面最上方、字号最大的文字。这是必填字段，不要输出 null，如果看到任何类似标题的文字就原样填写。
...
- `business_items`：经营项目。
- `valid_from`：有效期起始日期；仅在证照明确出现“有效期自/有效期起/自...起”等开始含义，或备案凭证出现“备案日期”“备案时间”且该日期表示备案开始生效时间时抽取。
- `valid_to`：有效期截止日期；未识别到时按长期有效处理。
- `issue_authority`：发证机关。
- `issue_date`：签发日期、发证日期、核发日期。
...
- `business_items` 输出数组。
- 食品经营许可证或仅销售预包装食品备案凭证中的“备案日期”“备案时间”应优先作为 `valid_from`，不要因为字段名不是“有效期开始”而漏抽。
- “有效期至”“有效期止”“截止日期”“到期日期”只能抽取为 `valid_to`，不得同时填入 `valid_from`。
- “签发日期”“发证日期”“核发日期”优先抽取为 `issue_date`；当前端或字段比对需要展示有效期开始但证照没有明确开始日期时，可用 `issue_date` 作为展示兜底，但 OCR 抽取不得把 `valid_to` 复制为 `valid_from`。
- 日期字段必须规范为 `YYYY-MM-DD` 后输出，例如 `2023年11月09日` 输出为 `2023-11-09`，`2028年11月08日` 输出为 `2028-11-08`；无法确定输出 `null`。

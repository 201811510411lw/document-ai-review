...
def business_license_review_state_from_workflow_state(
    workflow_state: BusinessLicenseWorkflowState,
) -> ReviewState:
    ...
    source_evidence = dict(workflow_state.get("source_evidence", {}))
    skill_rule_review_metadata = workflow_state.get("skill_rule_review_metadata", {})
    if skill_rule_review_metadata:
        source_evidence = {
            **source_evidence,
            "skill_rule_review_metadata": skill_rule_review_metadata,
        }
...
def _decision_from_workflow_state(
    workflow_state: BusinessLicenseWorkflowState,
) -> ReviewDecision:
    status = workflow_state.get("status")

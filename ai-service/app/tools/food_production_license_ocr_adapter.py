...
class QwenOcrFoodProductionLicenseAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = (
            model
            or os.environ.get("FOOD_PRODUCTION_LICENSE_QWEN_OCR_MODEL")
            or os.environ.get("FOOD_PRODUCTION_LICENSE_FILE_RECOGNITION_MODEL")
            or os.environ.get("BUSINESS_LICENSE_QWEN_OCR_MODEL", "")
        )
        self.api_key = api_key
        self.base_url = base_url or os.environ.get("OPENAI_BASE_URL")
        self.timeout = timeout or float(
            os.environ.get("FOOD_PRODUCTION_LICENSE_QWEN_OCR_TIMEOUT_SECONDS", "90")
        )
        ...
        self.max_pages = int(
            os.environ.get("FOOD_PRODUCTION_LICENSE_QWEN_OCR_MAX_PAGES", "5")
        )
    ...
    def extract_text(self, source: Any) -> dict[str, Any]:
        ...
        try:
            for page_number, data_url in enumerate(page_data_urls[: self.max_pages], start=1):
                    ...
                    messages=[
                        {
                            ...
                            "content": [
                                {
                                    "type": "text",
                                    "text": food_production_license_qwen_ocr_prompt(),
                                },
                ...
                if page_results[-1]["is_food_production_license"]:
                    break
        except Exception as error:
            return self._error(
                "failed",
        ...
        selected = _select_food_production_license_page(page_results)
        metadata = {
            ...
            "attempts": attempts,
            "structured_extraction": "qwen_food_production_license_ocr_parse",
            "raw_response_suppressed": True,
            "selected_page": selected.get("page") if selected else None,
            "ignored_pages": [
                {"page": item["page"], "reason": "not_food_production_license_page"}
                for item in page_results
                if selected is None or item["page"] != selected["page"]
            ],
        }
        if selected is None:
            metadata["error_code"] = "QWEN_OCR_FOOD_PRODUCTION_LICENSE_PAGE_NOT_FOUND"
            return {
...
class QwenOcrWithAliyunFallbackFoodProductionLicenseAdapter:
    implementation_status = "configured"
    ...
    def __init__(
        ...
        *,
        ...
        self.primary_adapter = primary_adapter or QwenOcrFoodProductionLicenseAdapter()
        self.fallback_adapter = fallback_adapter or AliyunOcrTextAdapter()
        self.fallback_text_parser = (
            fallback_text_parser or FoodProductionLicenseOcrTextParser()
        )

    def extract_text(self, source: Any) -> dict[str, Any]:
        primary_result = self.primary_adapter.extract_text(source)
        primary_validation = validate_food_production_license_ocr_result(primary_result)
...
class FoodProductionLicenseOcrTextParser:
    implementation_status = "configured"
    ...
    def __init__(
        ...
        *,
        ...
        self.model = (
            model
            or os.environ.get("FOOD_PRODUCTION_LICENSE_TEXT_PARSE_MODEL")
        ...
        self.base_url = base_url or os.environ.get("OPENAI_BASE_URL")
        self.timeout = timeout or float(
            os.environ.get("FOOD_PRODUCTION_LICENSE_TEXT_PARSE_TIMEOUT_SECONDS", "90")
        )
        self.max_attempts = max_attempts or int(
            os.environ.get(
    ...
    def extract_text(self, ocr_result: dict[str, Any]) -> dict[str, Any]:
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": food_production_license_ocr_text_parse_prompt(
                                    document_text
                                ),
                            }
                        ],
        ...
        parsed_metadata = {
            **metadata,
            ...
            "api": "chat.completions",
            "attempts": attempts,
            "structured_extraction": "food_production_license_aliyun_ocr_text_llm_parse",
            "raw_response_suppressed": True,
        }
        if not fields:
...
def validate_food_production_license_ocr_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if document_type not in {"food_production_license", "食品生产许可证"}:
        failure_reasons.append("document_type_invalid")

    if not _normalize_text(fields.get("producer_name")):
        failure_reasons.append("producer_name_missing")
...
def food_production_license_qwen_ocr_prompt() -> str:
    skill_text = _load_skill_text("food-production-license-review")
    return (
...
def food_production_license_ocr_text_parse_prompt(document_text: str) -> str:
    skill_text = _load_skill_text("food-production-license-review")
    return (
...
def _sanitize_food_production_license_fields(fields: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(sanitized.get("food_categories"), list):
        value = sanitized.get("food_categories")
        sanitized["food_categories"] = [value] if value else []
    return sanitized
...
def _select_food_production_license_page(
    ...
    candidates = [item for item in page_results if item.get("is_food_production_license")]
    if not candidates:
        return None
    return max(
        candidates,
        key=lambda item: _food_production_license_score(item["fields"], item["text"]),
    )


def _is_food_production_license_page(fields: dict[str, Any], text: str) -> bool:
    ...
    if "食品生产许可证" in compact:
        return True
    return bool(fields.get("license_no") and str(fields.get("license_no")).upper().startswith("SC"))


def _food_production_license_score(fields: dict[str, Any], text: str) -> int:
    compact = "".join((text or "").split())
    score = 0
    if "食品生产许可证" in compact:
        score += 5
    if fields.get("document_type") in {"food_production_license", "食品生产许可证"}:
        score += 4
    for key in ("producer_name", "credit_code", "license_no", "production_address", "valid_to"):
        if fields.get(key):
            score += 1
    return score

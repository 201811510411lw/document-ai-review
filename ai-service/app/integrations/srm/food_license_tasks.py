...
class FoodLicenseSourceTaskError(ValueError):
...
class FoodLicenseSourceTask(BaseModel):
...
def fetch_one_food_license_source_task(
    sql_client: SqlFetchClient,
    sql: str = DEFAULT_FOOD_LICENSE_SOURCE_SQL,
) -> FoodLicenseSourceTask | None:
    tasks = fetch_food_license_source_tasks(sql_client, sql)
    return tasks[0] if tasks else None


def fetch_food_license_source_tasks(
    sql_client: SqlFetchClient,
    sql: str,
) -> list[FoodLicenseSourceTask]:
    rows = sql_client.fetch_all(sql)
    tasks: list[FoodLicenseSourceTask] = []
    ...
    for row in rows:
        ...
        if not record.file_url:
            raise FoodLicenseSourceTaskError(
                "FOOD_LICENSE_SOURCE_URL_MISSING",
                "食品经营许可证来源记录缺少文件 URL",

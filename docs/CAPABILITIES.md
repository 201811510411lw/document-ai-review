...
- 支持 OA 以 `workflow_id + requestid` 精确定位来源，并按 `workflow_id + requestid + submission_version` 幂等触发后台自动审核；驳回重提的新版本会重新执行 OCR，最终结果推送到固定回调地址，并可使用专用 token 轮询。
- 支持 `standard` 和 `store_in_store` 两种审核模式。
- 单店对营业执照、烟草证和 OA 加盟商名称执行主体绑定校验；店中店自动区分持证主体营业执照与加盟店营业执照，对持证主体三信息和三证同址执行确定性规则，地址证明转人工复核。

...
- 明确 FastAPI、LangGraph、LangChain、Python 规则引擎和 SQLite / MySQL 的职责边界；
...
| Service | 创建审核任务，调用 LangGraph 工作流，协调结果保存和人工复核动作 |
| LangGraph | 编排食品安全证照检测 V1 的核心流程和节点状态流转 |
...
API 层不得直接实现食品安全证照规则判断。最终风险等级不得由 LLM 直接给出，必须由 Python 规则引擎结果汇总得到。
...
- Service 创建审核任务并记录任务初始状态；
- LangGraph 执行食品安全证照检测工作流；
- LangChain / LLM 负责字段抽取、结构化输出和摘要建议；
- Python 规则引擎负责规则判断；

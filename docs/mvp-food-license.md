...
规则实现位于 `ai-service/app/capabilities/food_license/rules/`，当前规则集版本为 `food-license-rules-v1`。
...
后续扩展食品经营许可证规则仍应放在 `app/capabilities/food_license/rules/`，通用规则基础设施继续保留在 `app/rules/`。

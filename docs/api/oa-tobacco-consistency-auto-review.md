...
- OA 专用入口立即返回 `processing`；领域结果和轮询保持四态，向当前三态 OA 接收端回调 `pass`、`reject` 或 `exception`。内部 `manual_review` 使用 `REVIEW_REQUIRES_MANUAL_REVIEW`、`retryable=false` 的 `exception` 传输投影，由 OA 停留并转人工处理。

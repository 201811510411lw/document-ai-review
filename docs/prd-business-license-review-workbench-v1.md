...
- 审核结果列表页；
- 审核结果详情页；
- 基础人工复核动作入口，支持轻量结论写回和审计留痕。

V1 API 范围：

- 查询营业执照审核结果列表；
- 查询单条营业执照审核详情；
- 人工复核提交接口，写回轻量复核结论并记录审计事件。
...
- Manual review writeback records a lightweight final reviewer decision and audit event; full workflow state machine and approval flow remain deferred to a later PRD.
...
- 人工复核页可以提交确认通过或驳回复核结论，提交后状态更新为人工已复核，并展示复核写回状态和审计事件。
...
- Full manual review state machine and approval workflow beyond the lightweight writeback endpoint.
...
`decision` 支持 `approved` 和 `rejected`。接口要求 bearer token，提交成功后将投影表状态更新为 `MANUAL_REVIEWED`、`needs_manual_review=false`，并写入 `BUSINESS_LICENSE_MANUAL_REVIEW` 审计事件。

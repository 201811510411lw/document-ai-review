...
| OA | 提供烟草证一致性审核的流程、门店和附件证据 | 当前不主动回写 OA，也没有 OA 专用 token/callback 入口 |
...
证照字段必须来自附件识别结果或人工确认，不能使用 OA 门店名称补造主体字段。当前接口可供 OA adapter 读取结果载荷，但不会主动推进 OA 流程、调用任意 callback URL 或提供 `X-OA-Token` 鉴权。

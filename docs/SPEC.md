...
- OA 自动审核要求显式传入正整数 `workflow_id`，并按 `workflow_id + requestid` 精确定位来源，使用稳定任务 ID 幂等执行；当前“烟草商品建档申请”流程传 `614`，领域结果、持久化和轮询区分 `pass`、`reject`、`manual_review` 和 `exception`。当前 OA callback 接收端为三态协议，传输投影将 `manual_review` 映射为带 `REVIEW_REQUIRES_MANUAL_REVIEW`、`retryable=false` 的 `exception`，不改变领域结论。字段差异 `0..2` 项时返回 `pass` 并携带结构化差异进入下一节点，达到 3 项时返回 `reject`。
